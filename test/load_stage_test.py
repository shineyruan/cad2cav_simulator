...
class UsdLoadSample:
  ...
  def load_stage(self, args):
    ...
    if result is False:
      ...
      return False
    self.usd_path = os.path.join(nucleus_server, args.usd_path)
  ...
  def configure_bridge(self, json_file: str = "isaacsim.app.json"):
    ...
    ext_id = ext_manager.get_enabled_extension_id(
  ...
  def create_reb_camera(self, cameraIndex, name, width, height):
    ...
    result, reb_camera_prim = omni.kit.commands.execute(
        ...
        segmentation_output_channel="encoder_segmentation_{}".format(
            cameraIndex),
...
if __name__ == "__main__":
  ...
  parser.add_argument(
      ...
      type=int,
      default=60,
      help="Total walltime in seconds to calculate average FPS for")
  parser.add_argument(
      "--add_rebcamera",
      nargs="*",
      type=str,
